...
export function createHtmlContentForGame(config: GameConfig): string {
  ...
  const gameLogic = `
        ...
        switch (gameType) {
          case 'flappy-bird': {
            ...
            function jump() {
                ...
                if (!gameStarted) handleFirstInput();
          ...
          case 'whack-a-mole': {
            ...
            canvas.addEventListener('touchstart', handleTouchWhack);
            break;
          }
          case 'match-3': {
    ...
    function handleFirstInput() {
        if (!gameStarted) {
            gameStarted = true;
            if(gameType === 'whack-a-mole' && 'startTimer' in window) {
                window.startTimer();
            }
            if (gameLoop) {
                requestAnimationFrame(gameLoop);
    ...
    canvas.addEventListener('touchstart', (e) => { e.preventDefault(); handleFirstInput(); });
